...
export function mostrarTelaAlunoPrincipal() {
...
export function mostrarTelaAlunoDetalhes() {
  const principal = document.getElementById('tela-aluno-principal');
  const detalhes = document.getElementById('tela-aluno-detalhes');
  if (principal) principal.style.display = 'none';
  if (detalhes) detalhes.style.display = 'block';
  import('./ranking.js').then(({ atualizarInfoAluno }) => {
    atualizarInfoAluno();
  });
  import('./game.js').then(({ desenharGraficoEvolucao }) => {
...
export function exibirToast(mensagem, tipo = 'info') {
  ...
  if (t) {
    ...
    if (tipo === 'erro') t.style.background = '#c0392b';
    else if (tipo === 'sucesso') t.style.background = '#27ae60';
    else if (tipo === 'aviso') t.style.background = '#f39c12';
    else t.style.background = '#e67e22';
...
export function atualizarTimerFase(milissegundos) {
  ...
  const alunoDetalhesTimer = document.getElementById('timer-fase-detalhes');
  ...
  if (alunoDetalhesTimer) {
    alunoDetalhesTimer.innerText = `⏱️ Tempo: ${timeStr}`;
    if (segundos < 60) alunoDetalhesTimer.classList.add('urgente');
    else alunoDetalhesTimer.classList.remove('urgente');
  }
}
...
export function aplicarTema() {
  let tema = localStorage.getItem(TEMA_KEY) || 'tema-escuro';
  if (!tema) { tema = 'tema-escuro'; localStorage.setItem(TEMA_KEY, tema); }
...
function atualizarIconeTema(tema) {
  ...
  btn.title = tema === 'tema-escuro' ? 'Alternar para tema claro' : 'Alternar para tema escuro';
}
...
export function carregarCorPrimaria() {
  let cor = localStorage.getItem(COR_PRIMARIA_KEY) || '#3b82f6';
  aplicarCorPrimaria(cor);
  document.querySelectorAll('.btn-cor').forEach(btn => btn.classList.toggle('selected', btn.dataset.cor === cor));
}
...
export function atualizarBannerAviso(aviso) {
  ...
  banner.innerHTML = `<span class="banner-aviso-icone">📢</span><span class="banner-aviso-texto">${aviso.mensagem}</span>`;
}

// ===== NÍVEL DE ESTRELAS (MINI) =====
export function atualizarNivelEstrelasMini() {
  const total = state.estrelas.total || 0;
  const nivel = getNivelPorEstrelas(total);
  const icone = document.getElementById('icone-nivel-mini');
  const nome = document.getElementById('nome-nivel-mini');
  if (icone) icone.textContent = nivel.icone;
  if (nome) nome.textContent = nivel.nome;
}

// ===== NÍVEL DE ESTRELAS (COMPLETO) =====
export function atualizarNivelEstrelasUI() {
  ...
  `;
  if (proximo) {
    const pct = Math.min(100, ((total - nivel.min) / (proximo.nivel.min - nivel.min)) * 100);
    html += `
      <div class="nivel-estrelas-barra">
        <div class="barra-fundo"><div class="barra-preenchida" style="width:${Math.round(pct)}%;"></div></div>
        <span class="texto-progresso">${Math.round(pct)}%</span>
      </div>
      <div class="nivel-estrelas-proxima-meta">🎯 Próximo nível: ${proximo.nivel.icone} ${proximo.nivel.nome} (faltam ${proximo.faltam} estrelas)</div>
    `;
    const dicas = {
    ...
    const dica = Object.keys(state.configEstrelas.acoes).find(key => state.configEstrelas.acoes[key] > 0 && total < 120);
    html += `<div class="nivel-estrelas-dica">💡 ${dica ? dicas[dica] : 'Continue jogando para acumular estrelas!'}</div>`;
  } else {
...
export function exibirModalResultados(dados) {
  ...
  if (ultimaPartida) evolucaoPontos = pontos - ultimaPartida.pontos;
  const badges = [];
  ...
  let mensagem = '';
  let mensagemEmoji = '';
  if (posicao === 1) { mensagem = '🏆 VOCÊ É O LÍDER! Continue assim para manter a coroa!'; mensagemEmoji = '👑'; }
  else if (posicao <= 3) { mensagem = '🥇 Você está no pódio! O topo está logo ali.'; mensagemEmoji = '🚀'; }
  else if (posicao <= 10) { mensagem = '🔥 Você está no Top 10! Continue pressionando!'; mensagemEmoji = '💪'; }
  else if (evolucaoPontos !== null && evolucaoPontos > 0) { mensagem = '📈 Você evoluiu! Continue subindo!'; mensagemEmoji = '📈'; }
  else if (evolucaoPontos !== null && evolucaoPontos < 0) { mensagem = '😅 Nem sempre é perfeito. A próxima partida será melhor!'; mensagemEmoji = '💪'; }
  else { mensagem = '🔄 Consistência é a chave. Continue firme!'; mensagemEmoji = '🔄'; }
  ...
  let statusVaga = dentroVagas ? `✅ Você está na zona de classificação (Top ${vagas})!` : (posicao > vagas ? `🎯 Faltam ${posicao - vagas} posição${(posicao - vagas) > 1 ? 'es' : ''} para entrar no Top ${vagas}!` : `📋 Você precisa de mais ${minPartidas - totalPartidas} partida${(minPartidas - totalPartidas) > 1 ? 's' : ''} para ser elegível.`);
    ...
    if (liderResultados.length > 0) {
      ...
      if (melhorLider && melhorLider.acertos > 0) {
        ...
        if (tempoMedio > 0 && velLider > 0) {
          ...
          if (diff < -0.01) velocidadeVsLider = `⚡ Você foi mais rápido que o líder nesta partida (${Math.abs(diff).toFixed(2)}s de vantagem)!`;
          else if (diff > 0.01) velocidadeVsLider = `⏱️ O líder ainda é mais rápido. Você está a ${diff.toFixed(2)}s do recorde dele.`;
          else velocidadeVsLider = `🤝 Você igualou a velocidade do líder!`;
        }
      }
    }
  }
  ...
  if (state.bonusVelocidadeConfig?.ativo) {
    ...
    if (bonus > 0) bonusMessage = `⚡ Você ganhou +${bonus} pontos de bônus por velocidade!`;
  }
  ...
  const modalHtml = `
    <div class="modal-resultados" id="modal-pos-jogo">
      ...
      <div class="mensagem-personalizada"><span class="emoji">${mensagemEmoji}</span> ${mensagem}${bonusMessage ? `<br><span style="font-size:16px;">${bonusMessage}</span>` : ''}</div>
      <div class="bloco"><div class="bloco-titulo">🎯 METAS E PROXIMIDADE</div><div class="card-metrica"><span class="label">${statusVaga}</span></div>${proximoColegaHtml ? `<div class="card-metrica"><span class="label">${proximoColegaHtml}</span></div>` : ''}<div class="card-metrica"><span class="label">📋 Partidas jogadas:</span><span class="valor">${totalPartidas} / ${minPartidas} ${totalPartidas >= minPartidas ? '✅ Elegível' : '⚠️ Faltam ' + (minPartidas - totalPartidas)}</span></div><div class="progresso-mini"><div class="fill" style="width:${Math.min(100, (totalPartidas/minPartidas)*100)}%;"></div></div></div>
      <div class="bloco"><div class="bloco-titulo">💪 SUPERAÇÃO PESSOAL</div><div style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:8px;">${badges.map(b => `<span class="badge-selo ativo">${b}</span>`).join('')}</div><div class="grid-2col">${evolucaoPontos !== null ? `<div class="card-metrica"><span class="label">📈 Evolução vs última partida:</span><span class="valor" style="color:${evolucaoPontos >= 0 ? '#4ade80' : '#f87171'};">${evolucaoPontos >= 0 ? '+' : ''}${evolucaoPontos} pts</span></div>` : ''}${tempoMedio > 0 ? `<div class="card-metrica"><span class="label">⚡ Tempo médio:</span><span class="valor">${tempoMedio.toFixed(2)}s</span></div>` : ''}</div></div>
      <div class="bloco"><div class="bloco-titulo">📊 RAIO-X TÁTICO</div><div class="grid-2col"><div class="card-metrica"><span class="label">🎯 Precisão:</span><span class="valor">${precisao.toFixed(0)}%</span></div><div class="card-metrica"><span class="label">⏱️ Tempo total:</span><span class="valor">${tempoTotal.toFixed(1)}s</span></div>${velocidadeVsLider ? `<div class="card-metrica" style="grid-column:1/-1;"><span class="label">⚡ Velocidade vs Líder:</span><span class="valor">${velocidadeVsLider}</span></div>` : ''}${projecaoPos ? `<div class="card-metrica"><span class="label">📈 Projeção de posição:</span><span class="valor">${projecaoPos}º (ritmo atual)</span></div>` : ''}</div></div>
      ${errosHtml}
      <div class="frase-encerramento">💡 A fase ainda não acabou! Sua próxima tentativa pode te levar ao topo.</div>
  ...
  document.getElementById('btn-fechar-modal')?.addEventListener('click', () => document.getElementById('modal-pos-jogo')?.remove());
